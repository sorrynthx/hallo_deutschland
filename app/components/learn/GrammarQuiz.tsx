...
function FillBlank({ item, onNext }: { item: GrammarItem; onNext: (correct: boolean) => void }) {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            {selected && (
                <div style={{ background: isCorrect ? 'rgba(78,205,196,.08)' : 'rgba(232,138,138,.08)', border: `1px solid ${isCorrect ? 'rgba(78,205,196,.3)' : 'rgba(232,138,138,.3)'}`, borderRadius: '10px', padding: '12px' }}>
            ...
            {selected && item.related_table?.length > 0 && (
                <div style={{ background: 'var(--bg-panel)', border: '1px solid rgba(255,255,255,.08)', borderRadius: '10px', overflow: 'hidden' }}>
                    <div style={{ padding: '8px 12px', fontSize: '9px', color: 'var(--mint)', fontFamily: 'Space Mono, monospace', fontWeight: 700, borderBottom: '1px solid rgba(255,255,255,.06)' }}>RELATED TABLE</div>
                    {item.related_table.map((row, i) => (
                        <div key={i} style={{ display: 'flex', padding: '8px 12px', gap: '12px', borderBottom: i < item.related_table.length - 1 ? '1px solid rgba(255,255,255,.04)' : 'none', background: row.form === item.answer || row.dative === item.answer ? 'rgba(78,205,196,.06)' : 'transparent' }}>
                            <span style={{ fontSize: '11px', color: 'var(--text-dim)', fontFamily: 'Space Mono, monospace', minWidth: '80px' }}>{row.pronoun}</span>
                            <span style={{ fontSize: '11px', color: row.form === item.answer || row.dative === item.answer ? 'var(--mint)' : 'var(--text-secondary)', fontWeight: 700 }}>
                                {row.form ?? row.dative}
                            </span>
                        </div>
                    ))}
...
function ConjugationTable({ item, onNext }: { item: GrammarItem; onNext: (correct: boolean) => void }) {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ background: 'var(--bg-panel)', border: '1px solid rgba(78,205,196,.2)', borderRadius: '10px', padding: '10px 14px' }}>
                <div style={{ fontSize: '9px', color: 'var(--mint)', fontFamily: 'Space Mono, monospace', fontWeight: 700, marginBottom: '4px' }}>CONJUGATION</div>
                <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)' }}>{item.topic}</div>
                <div style={{ fontSize: '10px', color: 'var(--text-secondary)', marginTop: '4px' }}>{item.topic_explanation}</div>
            </div>
            <div style={{ fontSize: '11px', color: 'var(--text-dim)', fontFamily: 'Space Mono, monospace', textAlign: 'center' }}>각 칸을 탭해서 활용형을 확인하세요</div>
            <div style={{ background: 'var(--bg-panel)', border: '1px solid rgba(255,255,255,.08)', borderRadius: '10px', overflow: 'hidden' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', padding: '8px 12px', borderBottom: '1px solid rgba(255,255,255,.1)', background: 'rgba(78,205,196,.06)' }}>
                    <span style={{ fontSize: '9px', color: 'var(--mint)', fontFamily: 'Space Mono, monospace', fontWeight: 700 }}>PRONOUN</span>
            ...
            {allRevealed && (
                <button className="btn btn-mint" style={{ padding: '14px', fontSize: '13px' }} onClick={() => onNext(true)}>다음 문제 →</button>
            )}
        </div>
    )
}
